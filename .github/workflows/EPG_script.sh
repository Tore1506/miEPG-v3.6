...
# NUEVO BLOQUE: Descomprimir epg_acumulado.xml.gz si existe
if [ -f epg_acumulado.xml.gz ] && [ ! -f epg_acumulado.xml ]; then
    echo " Descomprimiendo epg_acumulado.xml.gz → epg_acumulado.xml"
    gzip -d -c epg_acumulado.xml.gz > epg_acumulado.xml
fi
